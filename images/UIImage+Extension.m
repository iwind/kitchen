...
// by  Mario Klingemann
- (UIImage*) blur:(NSUInteger)inradius {
...
- (UIImage *) initWithSampleBuffer:(CMSampleBufferRef) sampleBuffer {
    ...
    if (self = [self initWithCGImage:quartzImage]) {
...
- (UIImage *) rotateWithOrientation:(UIInterfaceOrientation) orientation {
    ...
    if (orientation == UIInterfaceOrientationPortrait) {
        newImage = [UIImage imageWithCGImage:self.CGImage scale:1.0 orientation:UIImageOrientationRight];
    }
    else {
        newImage = self;
    }
